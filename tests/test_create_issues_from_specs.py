...
def test_load_spec_labels_optional(tmp_path):
    ...
    spec = load_spec(path)
    assert "labels" not in spec or spec.get("labels") is None
...
def test_run_skips_existing_issues(tmp_path, monkeypatch):
    ...
    def fake_fetch(repo):
        return {"feat: one"}  # first spec already exists
...
def test_run_dry_run_does_not_call_create(tmp_path, monkeypatch):
    write_spec(tmp_path, "f1.yaml", {"title": "feat: one", "body": "body1"})

    monkeypatch.setattr(
        "scripts.create_issues_from_specs.fetch_open_issue_titles", lambda repo: set()
    )
    create_spy = MagicMock()
    monkeypatch.setattr("scripts.create_issues_from_specs.create_issue", create_spy)
...
def test_run_all_specs_already_exist(tmp_path, monkeypatch):
    write_spec(tmp_path, "f1.yaml", {"title": "feat: one", "body": "body1"})

    monkeypatch.setattr(
        "scripts.create_issues_from_specs.fetch_open_issue_titles",
        lambda repo: {"feat: one"},
    )
    create_spy = MagicMock()
    monkeypatch.setattr("scripts.create_issues_from_specs.create_issue", create_spy)
