...
def test_creds_step_masks_github_app_token(creds_step):
    ...
    assert mask_idx is not None, "secrets must be masked with ::add-mask::"
    assert output_idx is not None, "secrets must be written to GITHUB_ENV"
    assert mask_idx < output_idx, "secrets must be masked before being written to env"
