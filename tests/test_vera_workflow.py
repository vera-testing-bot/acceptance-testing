...
def test_creds_step_masks_github_app_token(creds_step):
    ...
    output_idx = next(
        (i for i, l in enumerate(lines) if "GITHUB_ENV" in l and ">>" in l),
        None,
    )
    ...
    assert mask_idx < output_idx, "Credentials must be masked before being exported"


def test_run_agent_uses_app_token_for_gh_token(run_agent_step):
    env = run_agent_step["env"]
    gh_token = env.get("GH_TOKEN", "")
    assert "GITHUB_APP_TOKEN" in gh_token or "github_app_token" in gh_token, (
        "GH_TOKEN must prefer GitHub App token"
    )
...
def test_run_agent_uses_app_token_for_github_token(run_agent_step):
    ...
    assert "GITHUB_APP_TOKEN" in github_token or "github_app_token" in github_token, (
